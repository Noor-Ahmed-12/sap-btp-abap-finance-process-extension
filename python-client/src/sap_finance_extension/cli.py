...
def get_invoice_or_raise(
    ...
    """Return an invoice or raise a user-friendly CLI error."""

    stored_invoice = repository.get(invoice_uuid)

    if stored_invoice is None:
        raise CLIError(f"Invoice not found: {invoice_uuid}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="SAP finance invoice workflow demo"
    )
    ...
    subparsers.add_parser(
        "init",
        help="Initialize the local data store",
    )
    ...
    reopen_parser = subparsers.add_parser(
        "reopen",
        help="Reopen a rejected invoice",
    )
    reopen_parser.add_argument("invoice_uuid")

    post_parser = subparsers.add_parser(
        "post",
        help="Post an approved invoice",
    )
    post_parser.add_argument("invoice_uuid")

    subparsers.add_parser(
        "summary",
        help="Show finance summary",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "init":
            repository.storage_path.parent.mkdir(
                parents=True,
                exist_ok=True,
            )
        ...
        if args.command == "import-csv":
            ...
            for imported_invoice in result.successful:
                existing_invoices = repository.list_all()

                ensure_valid_invoice(
                    imported_invoice,
                    existing_invoices,
                )
                repository.add(imported_invoice)
                imported_count += 1
        ...
        if args.command == "list":
            ...
            for stored_invoice in invoices:
                print(
                    f"{stored_invoice.invoice_uuid} | "
                    f"{stored_invoice.processing_status.value} | "
                    f"{stored_invoice.vendor_id}"
                )
        ...
        if args.command == "validate":
            ...
            existing_invoices = [
                existing_invoice
                for existing_invoice in repository.list_all()
                if existing_invoice.invoice_uuid
